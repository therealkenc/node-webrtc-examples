...
tape('ConnectionManager', (t) => {
  ...
  t.test('.getConnection(id)', (t) => {
    ...
    t.equal(
      connectionManager.getConnection('foo'),
      null,
      '.getConnection(id) with an unknown id returns null'
    );
    t.equal(
      connectionManager.getConnection(connection.id),
      connection,
      '.getConnection(id) with a known id returns a Connection'
    );
    connection.close();
    t.equal(
      connectionManager.getConnection(connection.id),
      null,
      '.getConnection(id) returns null once the Connection with that ID closes'
    );
    t.end();
  });
  ...
  t.test('.getConnections()', (t) => {
    ...
    t.deepEqual(
      ...
      [],
      '.getConnections() initially returns an empty array'
    );
    const connection1 = connectionManager.createConnection();
    t.deepEqual(
      connectionManager.getConnections(),
      [connection1],
      '.getConnections() returns the expected result when 1 Connection is open'
    );
    const connection2 = connectionManager.createConnection();
    t.deepEqual(
      connectionManager.getConnections(),
      [connection1, connection2],
      '.getConnections() returns the expected result when 2 Connections are open'
    );
    connection1.close();
    connection2.close();
    t.deepEqual(
      connectionManager.getConnections(),
      [],
      '.getConnections() returns an empty array when all Connections are closed'
    );
    t.end();
  });
